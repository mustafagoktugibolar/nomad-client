...
const FilterBar: React.FC<FilterBarProps> = ({
  ...
  // Helper for passport localization
  const getPassportLabel = (p: Passport | null) => {
    if (!p) return t('passport_default');
    const name = p.country;
    if (name.includes("Bordo")) return t('passport_bordo');
    if (name.includes("Yeşil")) return t('passport_yesil');
    if (name.includes("Gri")) return t('passport_gri');
    if (name.includes("Siyah")) return t('passport_siyah');
    return name;
  };
  ...
  const buttons = [
    {
      ...
      label: getPassportLabel(selectedPassport),
      onClick: onPassportClick,
    },
    {
      type: "popover",
      icon: <Briefcase className="w-5 h-5 text-blue-600" />,
      label: t('filter_travel_reason'),
        ...
        <div className="md:hidden w-full flex flex-col items-center">
          ...
          >
            {isMobileExpanded ? (
              ...
              <>
                <Briefcase className="w-4 h-4 text-blue-600" /> {t('filter_filters')}
              </>
            )}
          </button>
        </div>
